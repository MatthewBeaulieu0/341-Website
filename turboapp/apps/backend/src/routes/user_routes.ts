...
import { get_user_by_id, create_new_user, get_user_cart, add_product_to_cart, delete_product_from_cart } from "../controllers/user_controllers";
...
user.delete("/id/:user_id/shopping_cart/", async (req: Request, res: Response) => {
    try{
        console.log(req.body)
        let product_id = parseInt(req.body.product_id);
        let user_id = parseInt(req.params.user_id);
        let status, data = await delete_product_from_cart(user_id, product_id);
        res.json({data})
        if (status == 200) {res.sendStatus(200);}
        if (status == 404) {res.sendStatus(404);}
    }
